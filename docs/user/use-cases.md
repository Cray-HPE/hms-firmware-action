...
1.  Create a JSON file with the command parameters required for updating the BMC firmware.
    ...
    For example, if the Chassis Management Module \(cC\), name the file chassisBMC.json.
...
2.  Create a JSON file with the command parameters required for updating the CMM firmware.

    For example, the file could be named chassisBMC.json.
    ...
    "command": {
      ...
      "restoreNotPossibleOverride":true,
      "timeLimit":10000,
      "description":"Update Cray Chassis Management Module controllers dry run"
     } 
    }
    ```
